...
class PLL:
    def __init__(self, phase_error_gain=0.01, lock_threshold=0.1):
        self.phase_error_gain = phase_error_gain
        self.lock_threshold = lock_threshold
        self.clock_offset = 0
        self.locked = False

    def adjust_offset(self, phase_error):
        adjustment = self.phase_error_gain * phase_error
        self.clock_offset += adjustment
        if abs(phase_error) < self.lock_threshold:
            self.locked = True
        else:
            self.locked = False

    def get_adjusted_time(self, current_time):
        return current_time + self.clock_offset

class Transmission_State:
    def __init__(self, initial_state, initial_time, time_slot= random.randint(0, 255)):
        self.state =initial_state #True/False, has it been seen
        self.next_transmission_time =initial_time +  TRIGGER_MICROSECONDS * 256
        self.pll = PLL()
        self.time_slot = time_slot
        self.free_time_slots = [True] * 256
        self.free_time_slots[self.time_slot] = False
        self.next_time_slot = self.time_slot
        self.received_code = [self.time_slot, bit_invert(self.time_slot), self.time_slot, bit_invert(self.time_slot)] #[random.randint(0, 255),random.randint(0, 255),random.randint(0, 255),random.randint(0, 255)] #[255, 5, 255, 5]
        self.transmitted_code = self.received_code
        
    def adjust_transmission_time(self, current_time):
        adjusted_time = self.pll.get_adjusted_time(current_time)
        self.next_transmission_time = adjusted_time + TRIGGER_MICROSECONDS * 256  

async def evaluate_transmission(transmission_state, color_state):
    t_now = time.monotonic()
    phase_error = t_now - transmission_state.next_transmission_time
    transmission_state.pll.adjust_offset(phase_error)
    transmission_state.adjust_transmission_time(t_now)
...
#         transmission_state.next_transmission_microseconds = TRIGGER_MICROSECONDS + t_now + time_offset
    ...
    if transmission_state.pll.locked:
         print("timeslot adjustment")
         color_state.color_choice = (color_state.color_choice + transmission_state.received_code[2]) // 2
         transmission_state.transmitted_code[2] = color_state.color_choice
         transmission_state.transmitted_code[3] = bit_invert(color_state.color_choice)
    #color_chase(pixels_DIO_Tornado,tornado_pixels,colorwheel(transmission_state.received_code[2]), 0.0)
    #transmission_state.color_choice = (color_state.color_choice+transmission_state.received_code[2])/2
...
#                         print(transmission_state.free_time_slots[transmission_state.received_code[0]])
                        transmission_state.free_time_slots[transmission_state.received_code[0]] = False
                        phase_error = time.monotonic() - transmission_state.next_transmission_time
                        transmission_state.pll.adjust_offset(phase_error)
                        pin_led.value = True
#                         print("success")
                    else:
...
async def evaluate_timeslot(transmission_state):
    while True:
        t_now = time.monotonic()
        next_time = transmission_state.pll.get_adjusted_time(t_now) + TRIGGER_MICROSECONDS * transmission_state.time_slot
        ...
        transmission_state.next_time_slot = (transmission_state.next_time_slot + 1) % 256
        await asyncio.sleep(next_time / 100)
#         t_now = time.monotonic()       
...
async def color_chase(pixels,num_pixels, color, color_state):
    for i in range(num_pixels):
        pixels[i] = color
        pixels.show()
        await asyncio.sleep(color_state.wait)        
    await asyncio.sleep(0)

async def color_cycle(color_state):
    while True:
        await color_chase(pixels_DIO_Tornado,tornado_pixels,colorwheel(color_state.color_choice), color_state)   
        await asyncio.sleep(0)


async def rainbow_cycle(pixels,num_pixels,controls):
    while True:
...
async def monitor_buttons(color_state,transmission_state):
    with keypad.Keys(
        ...
        while True:
            ...
            if key_event and key_event.pressed:
                if key_event.key_number == 0:
                    ...
                    if color_state.color_choice > 255:
                        color_state.color_choice = 0
                    else:
                        color_state.color_choice +=5
                else:
                    print("BTN_1 is down")
                    
                    if color_state.color_choice == 0:
                        color_state.color_choice = 255
                    else:
                        color_state.color_choice -=5
                transmission_state.transmitted_code[2] = color_state.color_choice
                transmission_state.transmitted_code[3] = bit_invert(color_state.color_choice)
...
async def status_text(transmission_state, color_state):
    while True:
        #$print(transmission_state.initial_state)
        print("next_transmission_microseconds:",transmission_state.next_transmission_time)
      ...
      #  print("".join(map(str,[bin(i) for i in transmission_state.free_time_slots])))
        ...
        print("color choice",color_state.color_choice)
        print("recieved code",transmission_state.received_code)
        print("transmitted code",transmission_state.transmitted_code)
        print("PLL status", transmission_state.pll.locked)
        print("PLL clock_offset", transmission_state.pll.clock_offset)
        await asyncio.sleep(10)
async def main():
    t = time.monotonic()
    transmission_state = Transmission_State(False,t)
    color_state = Color_Controls()
    while True:
        ...
        transmission_task = asyncio.create_task(evaluate_transmission(transmission_state,color_state))
        timeslot_task = asyncio.create_task(evaluate_timeslot(transmission_state))
        recieve_task  = asyncio.create_task(recieve_transmission(transmission_state))
        ...
        status_task = asyncio.create_task(status_text(transmission_state,color_state))
        button_task = asyncio.create_task(monitor_buttons(color_state,transmission_state))
        tornado_task = asyncio.create_task(color_cycle(color_state)) #rainbow_cycle(pixels_DIO_Tornado,tornado_pixels,0)#
        sao_h_task = asyncio.create_task(rainbow_cycle(pixels_SAO_H,SAO_2024_pixels,color_state))  # Increase the number to slow down the rainbow
        sao_45_task = asyncio.create_task(rainbow_cycle(pixels_SAO_45,SAO_2024_pixels,color_state))  # Increase the number to slow down the rainbow
        # This will run forever, because neither task ever exits.
        await asyncio.gather(tornado_task, sao_h_task,sao_45_task,button_task,timeslot_task,recieve_task,status_task,transmission_task)#,blink_task)
