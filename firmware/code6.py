...
encoder = asyncio.create_task(adafruit_irremote.GenericTransmit(header=[9582, 4529], one=[559, 559], zero=[559, 1699], trail=105))
...
class Transmission_State:
    def __init__(self, initial_state, initial_time):
        self.state=initial_state
        self.next_transmission_microseconds = TRIGGER_MICROSECONDS * 255 + initial_time
        self.time_slot = random.randint(0, 255)
        self.free_time_slots = [True] * 255
        self.free_time_slots[self.time_slot] = False
        self.next_time_slot = self.time_slot
        self.color_choice = (random.randint(0, 5))
...
async def evaluate_timeslot(transmission_state):
    while True:
        t_now = time.monotonic()
        print("transmission slot: ", transmission_state.next_time_slot)
        ...
        if transmission_state.next_time_slot == transmission_state.time_slot:
            await encoder.transmit(pulseout, transmission_state.transmitted_code)
        ...
        asyncio.sleep(transmission_state.next_transmission_microseconds)
...
async def main():
    ...
    while True:
        ...
        tornado_task = color_cycle(transmission_state)
        ...
        await asyncio.gather(tornado_task, sao_h_task,sao_45_task,button_task,timeslot_task,blink_task)#,recieve_task,timeslot_task,transmission_task)
